    ...
    def process(self):
        ...
        try:
            for idx in self.excel_data.index:
                ...
                try:
                    # Either 'invalid phone' alert or  whatsapp message box pop up depending on if the phone num is valid or not 
                    ...
                    logging.error('Exception while processing %s', contact_number)
                    logging.error(exp)
                    failCnt += 1
                    df = pd.DataFrame([[failCnt, name, contact_number]])
                ...
                if element.tag_name == 'span':
                    logging.info('Phone number valid: %s', contact_number)
                else:
